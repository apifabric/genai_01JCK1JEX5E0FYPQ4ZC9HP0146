...
resourcename="skishopmanagement"  # lower case, only
resourcegroup="skishopmanagement_rg"
...
# git clone https://github.com/apilogicserver/skishopmanagement.git
# cd skishopmanagement
...
az group create --name skishopmanagement_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group skishopmanagement_rg --sku S1 --is-linux
az container create --resource-group skishopmanagement_rg --name skishopmanagement --image apilogicserver/skishopmanagement:latest --dns-name-label skishopmanagement --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//skishopmanagement.westus.azurecontainer.io:5656'
