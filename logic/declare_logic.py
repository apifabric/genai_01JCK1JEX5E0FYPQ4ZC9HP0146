...
def declare_logic():
    ...
    # The customer's balance is the sum of the total amounts of their rental transactions where the return_date is null.
    Rule.sum(derive=Customer.balance, as_sum_of=RentalTransaction.amount_total, where=lambda row: row.return_date is None)

    # The total amount for a rental transaction is calculated based on stock quantity and price per day.
    Rule.formula(derive=RentalTransaction.amount_total, as_expression=lambda row: row.stock_quantity * row.price_per_day)

    # Customers can't have a balance that exceeds their credit limit.
